...
def normalize(nparray, order=2, axis=0):
    ...
    norm = np.linalg.norm(nparray, ord=order, axis=axis, keepdims=True) # 求矩阵范数
    return nparray / (norm + np.finfo(np.float32).eps) # 防止norm 为 0
...
def compute_dist(array1, array2, type='euclidean'):
  ...
  """
    ...
    if type == 'cosine':
        ...
        squared_dist[squared_dist < 0] = 0 # 矩阵中负数置为0
